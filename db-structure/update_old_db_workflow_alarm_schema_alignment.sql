-- Align pending_alarms workflow with your alarms table schema.
-- Your table uses: alarm_id (PK), scheduled_for (due time), alarm_target (device/target), status.
-- Run this after update_old_db_workflow_alarm_templates.sql.

-- 1) Fix custom query: use scheduled_for for "due" time (your next_alarm_time is null).
...
-- 2) Fix process_pending_alarms workflow: map alarm_id and device_id from your columns.
--    alarm_id <- record.alarm_id, device_id <- record.alarm_target (for push-to-device when set).
