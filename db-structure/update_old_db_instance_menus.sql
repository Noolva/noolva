-- Instance menus + per-client render config (client apps), separate from Noolva console public.menus.
...
-- Developer Console menu: Instance menus (Noolva console)
DO $$
DECLARE
    dev_id INT;
    uid INT;
BEGIN
    ...
    IF dev_id IS NULL THEN
        RAISE NOTICE 'apps.developer_console not found; skip instance menus menu insert';
        RETURN;
    END IF;
