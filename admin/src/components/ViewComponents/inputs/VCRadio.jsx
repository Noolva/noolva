      ...
      if (optionsMode === "collections" || optionsMode === "foreign_key") {
        ...
        try {
          ...
          if (optionsMode === "collections") {
            if (!collectionId) {
              console.warn(`VCRadio: collection_id is required for collections mode`);
              setOptions([]);
              setLoading(false);
              return;
            }
            ...
            if (!tableColumn) {
              console.warn(`VCRadio: table_column is required for foreign_key mode`);
              setOptions([]);
              setLoading(false);
              return;
            }
          ...
          console.error(`VCRadio: Failed to fetch options for ${input_values.name}:`, error);
          setOptions([]);
        } finally {
          setLoading(false);
        }
